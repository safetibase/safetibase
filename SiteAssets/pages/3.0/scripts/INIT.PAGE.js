$(document).ready(function () {
  ...
  function init(){
    ...
    $('#tpos-page').load('../3.0/html/layout.html',function(){
        ...
        setupleftnav(); // feels a little out of place withou tthe left nav, almost like its obvious there was supposed to be something there, if we break the leftnav but keep it then  we atleast get the username showing up
        ...
        //This pretty much works, so we just check (if  newHazard queryparam=== True) call this else don't, then we have the new hazard form. Same problem as with existing hazards is  the search here box and the sharepoint
        //banner at the top aren't the prettiest and some people might not want it there idk if we can get rid of the banner though, that could be a problem. 
        //We might have to hijack init for existing hazards as well, if we want to get rid of stuff like thesearch bar
        //search bar is added a few lines up and we could easily get rid of it 
  
        //These two lines hide any sharepoint related stuff, which could be nice im sure people might not want to see them  
  ...
  function setupleftnav(){
    ...
    if (urlParams.get("ID") || urlParams.get("newHazard")) {
        $('#tpos-nav').html('<div class="tpos-area-title">'+unm()+'</div><div id="user_roles" class="tpos-area-content hide"></div>'); 
    } else {
  ...
  function setupmainareastats(){
    ...
    $('#stats').load('../3.0/html/stats.tbl.html',function(){
        //this could be a good place to check for existence of the ID query parameter, if it exists we just override the cdmdata call with our own stuff
        //one potential problem I see is that if we move the check out of cdmdata.get i think we'll see all the leftNav stuff which was previously breaking due to us overriding cdmdata.get, and i was right about that 
        //we could combine the two, pass the parameter to both init.page.js and cdm data controller , that way we could do any removal of things we don't want to see in init.page.js
        //HArdcoding in cdmdata.get messes us up  when attempting to edit fields need another approach, or perhaps consume the query parameters
        urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get("ID")) {
            cdmdata.get('cdmHazards', "ID eq " + urlParams.get("ID"), "Modified desc", "hazards-search", 'statstbl', '');
        } else {
  ...
  function setupuserstats(r,c,s){
    ...
    $('#userstats').load('../3.0/html/stats.div.html',function(){
        ...
        if(r=='Site manager'){
            cdmdata.getQuickCount('cdmHazards',3,'cdmCurrentStatus eq \'Under site manager review\' and cdmSite/Title eq \''+s+'\'','RAMS hazards you could review','c','blue',null);
        ...
        cdmdata.getQuickCount('cdmHazards',13,'cdmCurrentStatus eq \'Under site manager review\' and cdmHazardOwner/Title eq \''+c+'\'','Under site manager review','csm','blue',null);
  ...
  function setupEditableHazards(){ // designers and sub contractor reps only
    $('#tpos-main').html('');
    // $('.dataset').removeClass('active');
    $('#stats').remove();
    $('#systemstats').remove();
    ...
    $.each(ds,function(){
        ...
        console.log(tc);
        if(tjs=='Designer'||tjs=='Sub contractor rep'){
            var c=tc;
  ...
  function setupReviewableHazards(a){
    $('#tpos-main').html('');
    // $('.dataset').removeClass('active');
    $('#stats').remove();
    $('#systemstats').remove();
    ...
    $.each(ds,function(){
        ...
        //     cst='Under pre-construction review';
        //     q=' and cdmSite/Title eq \''+ts+'\' and cdmCurrentStatus eq \''+cst+'\'';
        //     cst1='Under site manager review';
