function activateDatasets(){
    $('.dataset').off('click').on('click',function () {
...
function activateGlobalNav(){
    $('.tgn-btn').off('click').on('click',function(){
        var action=$(this).data('action');
        $('.dataset').removeClass('active');
        if(action=='init'){
            setupmainareastats();
...
function activateHazardOwnerEdit(){
    $('.cell-ownereditable').off('click').on('click',function(){
        var ii=$(this).attr('id');
        var ia=ii.split('_');
        var id=ia[1];
        var is=ia[2];
...
function activateHazardEdits(){
    $('.cell').off('click').on('click',function(){
        ...
        var hi=$(this).parents('.row-hazard').attr('id');
        var hia=hi.split('_');
        var id=hia[1];
        hzd=id;
        ...
        if(o=='<span class="clr_5">Unassigned</span>'){
            var warning='<div class="clr_5_active">Before the hazard can be edited and mitigated, an owner has to be assigned.</div>';
            gimmepops('Assigning an owner',warning+'<div id="popscontentarea"><i class="fa fa-spinner fa-spin"></i> Loading data</div>');
            
            if(hc=='RAMS hazard'){
                cdmdata.get('cdmCompanies','cdmCompanyRole/Title eq \'Sub contractor\'',null,'frmsel_owner',hc);
            }else{
                cdmdata.get('cdmCompanies','cdmCompanyRole/Title eq \'Design house\'',null,'frmsel_owner',hc);
            }
    

        }else{
            if(!uce){toastr.error('Not permitted');}else{
                flds=$(this).attr('class').split(' ');
                ...
                if(fld=='cdmPWElement'){
                    gimmepops('Assigning an element','<div id="popscontentarea"><i class="fa fa-spinner fa-spin"></i> Loading data</div>');
                ...
                if(fld=='cdmHazardDescription'){
                    ...
                    var txtbox='<div><textarea id="txtform" rows="6" cols="60">'+existingTxt+'</textarea></div>';
                    var svBtn='<div class="tpos-left-btn sv-hazard" onclick="savetxt(\'cdmHazardDescription\');">Save</div>';
                    gimmepops('Describing the hazard','<div class="clr_3_active">A hazard is something with the potential to cause harm</div>'+txtbox+svBtn);
                }
                if(fld=='cdmRiskDescription'){
                    var existingTxt=$('#'+hi+' .cdmRiskDescription').html();
                    var txtbox='<div><textarea id="txtform" rows="6" cols="60">'+existingTxt+'</textarea></div>';
                    var svBtn='<div class="tpos-left-btn sv-hazard" onclick="savetxt(\'cdmRiskDescription\');">Save</div>';
                    gimmepops('Describing the risk','<div class="clr_3_active">What harm could be caused to whom or what?</div>'+txtbox+svBtn);
                }
                if(fld=='cdmMitigationDescription'){
                    var existingTxt=$('#'+hi+' .cdmMitigationDescription').html();
                    var txtbox='<div><textarea id="txtform" rows="6" cols="60">'+existingTxt+'</textarea></div>';
                    var svBtn='<div class="tpos-left-btn sv-hazard" onclick="savetxt(\'cdmMitigationDescription\');">Save</div>';
                    gimmepops('Our mitigation','<div class="clr_3_active">Completed actions to minimise the risks</div>'+txtbox+svBtn);
                }
                if(fld=='cdmStageMitigationSuggestion'&&hc!='RAMS hazard'){
                    var existingTxt=$('#'+hi+' .cdmStageMitigationSuggestion').html();
                    var txtbox='<div><textarea id="txtform" rows="6" cols="60">'+existingTxt+'</textarea></div>';
                    var svBtn='<div class="tpos-left-btn sv-hazard" onclick="savetxt(\'cdmStageMitigationSuggestion\');">Save</div>';
                    gimmepops('Your mitigation suggestion for '+stage,'<div class="clr_3_active">Suggested actions to minimise the risks</div>'+txtbox+svBtn);
                }
                if(fld=='cdmInitialRAG'||fld=='cdmResidualRAG'){
                    var rags='<div id="rags"></div>';
                    gimmepops('Assessing and applying project controls - Initial control','<div class="clr_3_active">Search for, select or suggest a project control applicable to the hazard / risk</div>'+rags,'pops-fullscreen');
                    $('#rags').load('../3.0/html/rag.tables.html',function(){
                        getJsonFromTxt('rag.txt').done(function (data) {
                            ...
                            for (var cc = 0; cc < cnt; cc++) {
                                ...
                                var tds = '<td><div class="padded">' + t + '</div></td><td><div class="padded">' + d + '</div></td>';
                                if (s == 'Red') {
                                    reda = '<tr class="rag rag-red" data-v="'+cc+'"><td>' + tds + '</tr>';
                                    $('#ragdatared').append(reda);
                                }
                                if (s == 'Amber') {
                                    amba = '<tr class="rag rag-amber" data-v="'+cc+'"><td>' + tds + '</tr>';
                                    $('#ragdataamber').append(amba);
                                }
                                if (s == 'Green') {
                                    grea = '<tr class="rag rag-green" data-v="'+cc+'"><td>' + tds + '</tr>';
                            ...
                            $('.svrag').click(function(){
                                ...
                                if(sggid=='ragu'){
                                    ...
                                    if(clr!='none'&&sgg!=''){
                                        var tdata = [];
                                        tdata.push(fld + '|' + 'RAGSG'+hzd+'^'+sgg+'^'+clr);
                                        cdmdata.update('cdmHazards',tdata,'frmedit_updateview');
                                        $('#pops').remove();
                                    ...
                                    var tdata = [];
                                    tdata.push(fld + '|' + 'None^No applicable project control found^NONE');
                ...
                if(fld=='cdmInitialRisk'||fld=='cdmResidualRisk'){
                    ...
                    $('#rsselector').load('../3.0/html/risk.selection.panel.html',function(){
                        $('#inirisk').append('<div class="tpos-left-btn sv-hazard" onclick="saversk(\''+fld+'\');">Save</div>');
                ...
                if(fld=='cdmHazardCoordinates'){
                    ...
                    var cdtbl='<table id="newcoordinates" class="width-350">'+cd+'<tr class="ctagnew"><td><input style="color:rgb(0,0,0)" type="number" id="nx" placeholder="x" ></td><td><input style="color:rgb(0,0,0)" type="number" id="ny" placeholder="y" ></td><td><input style="color:rgb(0,0,0)" type="number" id="nz" placeholder="z" ></td><td><div class="tpos-addbtn">+</div></td></tr><tr><td colspan="4"><div class="tpos-svbtn">Save and Close</div></td></tr></table>';
                ...
                if(fld=='cdmHazardTags'){
                    gimmepops('Assigning a hazard tag','<div id="popscontentarea"><i class="fa fa-spinner fa-spin"></i> Loading data</div>');
                    cdmdata.get('cdmTags','',null,'frmsel_htag',hc);

                }
                if(fld=='cdmUniclass'){
                    gimmepops('Assigning an uniclass','<div id="popscontentarea"><i class="fa fa-spinner fa-spin"></i> Loading data</div>');
...
function decCTag(ai,str){
    var st=str.split(',');
    var t='<tr id="ctagrow_'+ai+'" class="ctagd"><td>'+st[0]+'</td><td>'+st[1]+'</td><td>'+st[2]+'</td><td><div id="ctag_'+ai+'" data-ctag="'+str+'" class="tpos-delbtn">-</div></td></tr>';
    return t;
}
function decHTag(ai,str){
    var st=str.split(',');
    var t='<tr id="htagrow_'+ai+'" class="htagd"><td>'+st[0]+'</td><td>'+st[1]+'</td><td>'+st[2]+'</td><td><div id="htag_'+ai+'" data-htag="'+str+'" class="tpos-delbtn">-</div></td></tr>';
    return t;
}
...
function activateCoordinatesSave(){
    $('.tpos-svbtn').off('click').on('click',function(){
        ...
        if(!x||!y||!z){
            toastr.error('Please enter all three numbers or press cancel (X) to close the pop up');
        }
    ...
    $('.tpos-svcbtn').off('click').on('click',function(){
        var tdata=[];
    ...
    $('.ctagd').off('click').on('click',function(){
...
function getctags(str){
    var ippanel='<div><input style="color:rgb(0,0,0)" type="number" placeholder="x" id="x" ><input style="color:rgb(0,0,0)" type="number" placeholder="y" id="y" ><input style="color:rgb(0,0,0)" type="number" placeholder="z" id="z" ></div>';
    var svbtn='<div class="tpos-svbtn">+</div><div class="tpos-svcbtn">Save & Close</div>';
    var ctags='';
    if(str){
        var strings = [];
        strings = str.split('^');
        ...
        for (var i = 0; i < strings.length; i++) {
            ...
            if (string != '') {
                ...
                t += '<tr class="ctagd" data-numero="'+i+'"><td>' + x + '</td><td>' + y + '</td><td>' + z + '</td></tr>';
            }
        }
        ctags='<div id="mngCoo"><table class="coordinatesTBL" style="width:45%;">'+t+'</table><div class="" id="h_'+hzd+'_fullcoor">'+str+'</div>'+ippanel+svbtn+'</div>';
...
function tposSelectOwner(lst,data,hzdt,trg){
    ...
    $('#popscontentarea').html('<table class="tpos-select-table">' + options + '</table>');
...
function tposSelectElement(lst,data,trg){
    var tlist = data.d.results;
    var options = '<tr><td class="hide" id="val_' + lst + '">0</td><td><input class="tpos-' + lst + '-select-input dvi" id="sel_' + lst + '" autofill="false" placeholder="Select an element ..."></td></tr>';
    for (var cc = 0; cc < tlist.length; cc++) {
        var it = tlist[cc];
        var itid = it.ID;
        var ittitle = it.Title;
...
function tposSelectTag(lst,data,trg){
    var tlist = data.d.results;
    var options = '<tr><td class="hide" id="val_' + lst + '">0</td><td><input class="tpos-' + lst + '-select-input dvi" id="sel_' + lst + '" autofill="false" placeholder="Select a tag ..."></td></tr>';
    for (var cc = 0; cc < tlist.length; cc++) {
        var it = tlist[cc];
        var itid = it.ID;
        var ittitle = it.Title;
...
function tposSelectUniclass(lst,data,trg){
    var tlist = data.d.results;
    var options = '<tr><td class="hide" id="val_' + lst + '">0</td><td><input class="tpos-' + lst + '-select-input dvi" id="sel_' + lst + '" autofill="false" placeholder="Select a uniclass ..."></td></tr>';
    for (var cc = 0; cc < tlist.length; cc++) {
        var it = tlist[cc];
        var itid = it.ID;
        var ittitle = it.Title;
...
function tposSelectPeer(lst,data,trg){
    var tlist = data.d.results;
    var options = '<tr><td class="hide" id="val_' + lst + '">0</td><td><input class="tpos-' + lst + '-select-input dvi" id="sel_' + lst + '" autocomplete="false" placeholder="Select a peer ... (optional)"></td></tr>';
    for (var cc = 0; cc < tlist.length; cc++) {
        var it = tlist[cc];
...
function hazardreviewbuttonaction(){
    $('.tpos-rvbtn').off('click').on('click',function(){
        toastr.success('what???');
        ...
        if(a=='initiatereview'){
            ...
            $('.pops-content').load('../3.0/html/review.initiation.form.html',function(){
                // cdmdata.get('cdmUsers',q,null,'frmsel_peer',null);
                $('.tpos-svbtn').off('click').on('click',function(){
                    var cmt=$('#cmt').val();
                    if(!cmt){cmt='no comment';}
                    var tdata=[];
                    nl=nnd+']'+user+']'+'requested peer review]'+cmt+'^';
            ...
            if(a=='peerreview'){
                ...
                $('.pops-content').load('../3.0/html/internal.design.review.form.html',function(){
                    $('.tpos-svbtn').off('click').on('click',function(){
                        ...
                        if(act=='change'){
                            if(cmt==''||!cmt||cmt==undefined){
                                toastr.error('Please use the comment box to explain your decision and outline the required changes.');
                            }
                            if(cmt){
                                nl=nnd+']'+user+']'+'requested change]'+cmt+'^';
                                hist = $('#h_'+hzd+'_cdmReviews').html();
                                // if(hist==''||!hist||hist==undefined){
                                //     hist='';
                                // }
                            ...
                            if(!cmt){cmt='no comment';}
                            nl=nnd+']'+user+']'+'completed peer review]'+cmt+'^';
                            hist = $('#h_'+hzd+'_cdmReviews').html();
                            // if(hist==''||!hist||hist==undefined){
                            //     hist='';
                            // }
                            ...
                            if(ns==1){
                                ...
                                tdata.push('cdmCurrentStatus|Under design manager review');
                            }
                ...
                $('.pops-content').load('../3.0/html/internal.design.review.form.html',function(){
                    $('.tpos-svbtn').off('click').on('click',function(){
                        ...
                        if(act=='change'){
                            ...
                            if(!cmt){cmt='no comment';}
                            nl=nnd+']'+user+']'+'completed design manager review]'+cmt+'^';
                            hist = $('#h_'+hzd+'_cdmReviews').html();
                            // if(hist==''||!hist||hist==undefined){
                            //     hist='';
                            // }
                            ...
                            if(ns==1){
                                tdata.push('cdmReviews|'+nl);
                                tdata.push('cdmCurrentStatus|Under pre-construction review');
                                tdata.push('cdmLastReviewDate|'+ind);
                                tdata.push('cdmLastReviewStatus|Design manager review - approved');
                                tdata.push('cdmLastReviewer|'+unm());
                            }else{
            ...
            if(a=='pcreview'){
                gimmepops('Undertake the pre-construction review','');
                $('.pops-content').load('../3.0/html/internal.design.review.form.html',function(){
                    $('.tpos-svbtn').off('click').on('click',function(){
                        var act=$(this).data('action');
                        var cmt=$('#cmt').val();
                        ...
                        if(act=='change'){
                            ...
                            if(!cmt){cmt='no comment';}
                            nl=nnd+']'+user+']'+'completed pre-construction review]'+cmt+'^';
                            hist = $('#h_'+hzd+'_cdmReviews').html();
                            // if(hist==''||!hist||hist==undefined){
                            //     hist='';
                            // }
                            ...
                            tdata.push('cdmLastReviewDate|'+ind);
                            tdata.push('cdmLastReviewStatus|Pre-construction review completed');
                            tdata.push('cdmLastReviewer|'+unm());
                            cdmdata.update('cdmHazards',tdata,'frmedit_updateview');
                            $('#pops').remove();
                ...
                $('.pops-content').load('../3.0/html/internal.design.review.form.html',function(){
                    $('.tpos-svbtn').off('click').on('click',function(){
                        ...
                        if(act=='change'){
                            ...
                            if(!cmt){cmt='no comment';}
                            nl=nnd+']'+user+']'+'completed lead designer review]'+cmt+'^';
                            hist = $('#h_'+hzd+'_cdmReviews').html();
                            // if(hist==''||!hist||hist==undefined){
                            //     hist='';
                            // }
                            ...
                            tdata.push('cdmReviews|'+nl);
                            tdata.push('cdmLastReviewDate|'+ind);
                            tdata.push('cdmLastReviewStatus|Lead designer review completed');
                            tdata.push('cdmLastReviewer|'+unm());
                            cdmdata.update('cdmHazards',tdata,'frmedit_updateview');
                            $('#pops').remove();
                ...
                $('.pops-content').load('../3.0/html/internal.design.review.form.html',function(){
                    $('.tpos-svbtn').off('click').on('click',function(){
                        ...
                        if(act=='change'){
                            ...
                            if(!cmt){cmt='no comment';}
                            nl=nnd+']'+user+']'+'completed site manager review]'+cmt+'^';
                            hist = $('#h_'+hzd+'_cdmReviews').html();
                            // if(hist==''||!hist||hist==undefined){
                            //     hist='';
                            // }
                            ...
                            tdata.push('cdmReviews|'+nl);
                            tdata.push('cdmLastReviewDate|'+ind);
                            tdata.push('cdmLastReviewStatus|Site manager review completed');
                            tdata.push('cdmLastReviewer|'+unm());
                            cdmdata.update('cdmHazards',tdata,'frmedit_updateview');
                            $('#pops').remove();
...
function toggleCollapse(){
    $('.vwshow').off('click').on('click',function () {
        $('.vwdefault').show();
        $('.vwhover').hide();
...
function toggleInfoPanel(){
    $('.revhis').off('click').on('click',function(){
    ...
    $('.hazhis').off('click').on('click',function(){
        // toastr.success('listening');
        var hi=$(this).parents('.row-hazard').attr('id');
        var hia=hi.split('_');
        var id=hia[1];
        ...
        if($('#h_'+hzd+'_changehistory').hasClass('hide')==1){
            ...
            cdmdata.get('cdmHazardHistory', 'cdmHazard eq \''+id+'\'', 'Modified desc', 'hazhistory','h_'+id+'_changehistory');
        }else{
