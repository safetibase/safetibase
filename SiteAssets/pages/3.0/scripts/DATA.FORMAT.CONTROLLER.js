  ...
  urbuttons: function(data, ftv, trg) {
    ...
    var role = [0, 0, 0, 0, 0],
      roles = [
        ...
        "Lead designer",
        "Sub contractor rep"
      ]; // designer, design manager, site manager, lead designer and sub contractor rep
    ...
    if (role[1] == 1) {
      ...
      var t = mkBtn(
        "review",
        "design",
        "Undertake design house reviews",
        "dmrHazard",
        "adh"
      );
      $("#user_roles").append(t);
  ...
  statstablerows: function(data, ftv, trg) {
    ...
    for (var dd = 0; dd < sa.length; dd++) {
      for (var ee = 0; ee < fa.length; ee++) {
        cdmdata.getQuickCount(
          "cdmHazards",
          cnt,
          "cdmSite/ID eq " + sa[dd] + " and " + fa[ee],
          stit[dd] + " - " + ft[ee],
          ftrg[ee] + sa[dd],
          fclr[ee],
          1
...
function mkLinkButtons(){
  cdmdata.get('XtraButtons', null, 'ID asc','xtrabuttons','tpos-nav');
}
...
function buildHazardListItem(h) {
  ...
  if (h.cdmHazardOwner.ID) {
      ...
      if (
        ...
        if (urct == h.cdmHazardOwner.ID) {
          ...
          if (h.cdmPWStructure.ID || h.cdmTW) {
            ...
            if (urat == "Design house manager" && uid() != h.Editor.ID) {
              ucandmreview = 1;
            }
          }
          if (h.cdmRAMS) {
            // is a RAMS hazard
            if (urat == "Sub contractor rep") {
              ucanedit = 1;
            }
          }
        ...
        if (uid() != h.Editor.ID) {
          if (urat == "Site manager" && urs == h.cdmSite.ID) {
            ucanprecon = 1;
          }
    ...
    if (ucansmreview == 1 && rvstatus == "requested site manager review") {
      ucansmreview = 2;
    }
  }
...
function printHazardRow(h) {
  ...
  if (h.cdmHazardOwner.Title) {
    ...
    for (var cc = 0; cc < ura.length; cc++) {
      ...
      if (hc != "ra") {
        ...
        console.log(isSM + ' - hasRAMS: '+hasRAMS);
        if(hasRAMS===0&& isSM===0){
          mkramsbtn='';
          lblramshazards='';
        }
        // if(role==='Site manager'&&h.cdmSMMitigationSuggestion==='Awaiting assessment'){
          if(isSM===1&&hasRAMS===0){
          mkramsbtn='<div class="tpos-svramsbtn mkramsbtn width-a" data-action="mkrams" id="mkramsbtn_'+h.ID+'" title="Only available to site managers to mitigate further via RAMS hazards">Add associated RAMS hazards</div>';
        }
        // if(role==='Site manager'&&h.cdmSMMitigationSuggestion!='Awaiting assessment'){
          if(isSM===1&&hasRAMS===1){
        ...
        if (requiresPCReview == 1) {
          (ruce = 2), (rucp = 2), (rucd = 2), (rucpc = 2), (rucl = 2);
  ...
  var requiredreviews =
    ...
    rucl +
    '"></div><div title="Site manager review" class="prm-cell rucs _' +
    rucs +
    '"></div></div>';
  ...
  permissions =
    ...
    ucl +
    '"></div><div title="Green - you can undertake site manager reviews for this hazard" class="prm-cell ucs _' +
    ucs +
    '"></div></div>';
  ...
  var myvar =
    ...
    "                    </td>" +
    '                    <td class="width-250">' +
    '                        <div class="lbl stagehide">Site manager\'s mitigation suggestion for ' +
    h.cdmStage.Title +
    "</div>" +
    "                    </td>" +
    '                    <td class="width-250">' +
    ...
    "                    </td>" +
    '                    <td class="width-250 fld">' +
    '                        <div class="cell cdmSMMitigationSuggestion stagehide pointer" title="Provided by site manager during the pre-construction review if required">' +
    h.cdmSMMitigationSuggestion +
    "</div>" +
    "                    </td>" +
    '                    <td class="width-250 fld">'+mkramsbtn+'</td>' +
    '                    <td class="width-250 fld"><div class="cell cdmParent ramsonly">' +
    h.cdmParent +
    "</div></td>" +
    "                </tr>" +
    "            </table>" +
    ...
    "                    </td>" +
    '                    <td class="width-250 fld">' +
    '                        <div class="cell cdmUniclass pointer" title="Click to manage Uniclass tags">' +
    unitags +
    "</div>" +
    "                    </td>" +
    '                    <td class="width-250 fld">' +
