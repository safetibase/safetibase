...
$WelcomeButton.Add_Click({
    ...
    $wshell2.Popup("Checking if required PowerShell modules are installed",2,"Installing Dependencies",0x0 + 0x40)
    $ModulesInstalled = Install-RequiredModules
    
    switch ($ModulesInstalled){
        "ImportExcel" {
            $wshell3 = New-Object -ComObject Wscript.Shell
            $EndInstall = $wshell3.Popup("PowerShell Module ImportExcel couldn't be installed.`nThis is required to perform installation.`nInstall module seperately and try again.`nRefer to:`nhttps://www.powershellgallery.com/packages/ImportExcel",0,"PowerShell Module install Failed",0x0 + 0x10)
            
        }
        "SharePointPnPPowerShellOnline" {
            $wshell4 = New-Object -ComObject Wscript.Shell
            $EndInstall = $wshell4.Popup("PowerShell Module SharePoint PnP couldn't be installed.`nThis is required to perform installation.`nInstall module seperately and try again.`nRefer to:`nhttps://docs.microsoft.com/en-us/powershell/sharepoint/sharepoint-pnp/sharepoint-pnp-cmdlets",0,"PowerShell Module install Failed",0x0 + 0x10)
